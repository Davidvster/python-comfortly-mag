# Taken from https://github.com/tejasa97/ECG-Signal-Processing
...
dataset = pd.read_csv(f"./data/ecg.csv",delimiter=DELIMETER)
y = [e for e in dataset.ecg_mv]
...
fig_td, ax_td = plt.subplots(2, 1, constrained_layout=True)
fig_td.suptitle('Time domain signals')
fig_fd, ax_fd = plt.subplots(2, 1, constrained_layout=True)
fig_fd.suptitle('Frequency domain signals')
ax_td[0].set_title('Before filtering')
ax_td[1].set_title('After filtering')
ax_fd[0].set_title('Before filtering')
ax_fd[1].set_title('After filtering')

# Plot non-filtered inputs
ax_td[0].plot(x, y, color='r', linewidth=0.7)
ax_fd[0].plot(xf, 2.0 / N * np.abs(yf[:N // 2]), color='r', linewidth=0.7, label='raw')
ax_fd[0].set_ylim([0, 0.2])
...
ax_fd[1].plot(xf, 2.0 / N * np.abs(yff[:N // 2]), color='g', linewidth=0.7)
ax_fd[1].set_ylim([0, 0.2])
ax_td[1].plot(x, y_filt, color='g', linewidth=0.7);

###Compute beats###
dataset['filt'] = y_filt
# Calculate moving average with 0.75s in both directions, then append do dataset
hrw = 1  # One-sided window size, as proportion of the sampling frequency
fs = 50  # The example dataset was recorded at 300Hz
